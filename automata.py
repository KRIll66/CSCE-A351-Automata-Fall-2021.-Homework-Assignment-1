...
class DFA:
    ...
    def __init__(self, *args, **kwargs):
        if (len(args) != 0) and (len(kwargs) != 0):
            raise Exception("Combinations of positional and keyword arguments not allowed")
        if len(kwargs) > 0:
            if "regex" in kwargs:
                ...
                if "Sigma" in kwargs:
                    ...
                    if len(kwargs) > 1:
                        raise Exception("Too many keyword arguments")
                    M = DFA(kwargs["DFA"])
            elif ("type" in kwargs) and ("Q" in kwargs) and ("Sigma" in kwargs) and ("delta" in kwargs) and ("q" in kwargs) and ("F" in kwargs):
                if len(kwargs) > 6:
                    raise Exception("Too many keyword arguments")
                M = DFA(kwargs["type"], kwargs["Q"], kwargs["Sigma"], kwargs["delta"], kwargs["q"], kwargs["F"])
            else:
            ...
            if len(args) == 1:
                if isinstance(args[0], str):
                    if len(args[0]) > 0:
                        Sigma = { a for a in args[0] if not a in { '(', ')', '|', '*' } }
                        M = DFA(args[0], Sigma)
                        self.__states = frozenset(M.__states)
                        self.__alphabet = frozenset(M.__alphabet)
                        self.__delta = M.__delta
                        ...
                        raise Exception("Empty regular expressions supported only when alphabet is given")
                    ...
                    raise Exception("Unimplemented case: argument must be a string or another DFA")
            elif (len(args) == 2):
                if isinstance(args[0], str):
                    if len(args[0]) == 0:
                        self.__states = frozenset({ 1, 2 })
                        self.__alphabet = self.__check_alphabet(frozenset(args[1]))
                        self.__delta = dict()
                        ...
                        if not(args[0][0] in frozenset(args[1])):
                            raise Exception("Character in regular expression not in alphabet")
                        if args[0][0] in { '(', '|', ')', '*' }:
                        ...
                        M = self.__regular_expression_to_dfa(args[0], self.__check_alphabet(frozenset(args[1])))
                        self.__states = frozenset(M.__states)
                        self.__alphabet = frozenset(M.__alphabet)
                        self.__delta = M.__delta
                        self.__start = M.__start
                    ...
                    M = args[0].__extend_alphabet(self.__check_alphabet(args[1]))
                    self.__states = frozenset(M.__states)
                    self.__alphabet = frozenset(M.__alphabet)
                    self.__delta = M.__delta
                    self.__start = M.__start
                    ...
                    raise Exception("Unimplemented case: argument not a string nor another DFA")
            elif len(args) == 6:
                if args[0] == "DFA":
                    ...
                    if isinstance(args[3], dict):
                        ...
                        for q in args[1]:
                            ...
                            if (not args[3](q, '') == None) and (not frozenset(args[3](q, '')) == frozenset()):
                                NFA_D[(q,'')] = args[3](q, '')
                    NFA_Q = args[1]
                    NFA_start = args[4]
                    NFA_Sigma = self.__check_alphabet(frozenset(args[2]))
                    NFA_delta, NFA_F = self.__remove_epsilon(NFA_Q, NFA_Sigma, NFA_D, args[5])
                    if self.__nfa_delta_without_epsilon_is_deterministic(NFA_delta, NFA_Q, NFA_Sigma):
                        self.__states = NFA_Q
                        self.__alphabet = NFA_Sigma
                        self.__start = NFA_start
                        self.__delta = { (q,a) : next(iter(NFA_delta[(q,a)])) for q in NFA_Q for a in NFA_Sigma }
                        self.__final = NFA_F
                        self.__check_consistance()
                    else:
                        self.__alphabet = NFA_Sigma
                    ...
                    raise Exception("Unimplemented case: first argument must be 'DFA' or 'NFA'")
            else:
                raise Exception("Unimplemented case: too many or too few arguments")
    ...
    def __check_consistance(self):
        ...
        for a in self.__alphabet:
            if not isinstance(a, str):
                raise Exception("Alphabet must be formed of 1-character strings")
            if len(a) != 1:
                raise Exception("Alphabet must be formed of 1-character strings")
            if a[0] in { '(', ')', '|', '*' }:
                raise Exception("Alphabet cannot include characters '(', ')', '|' or '*'")
        for q in self.__states:
            for a in self.__alphabet:
                if not ((q,a) in self.__delta):
                    raise Exception("Transition function must be complete, i.e. defined for all pairs (q,a)")
        ...
        for q in Q:
            if (q, '') in delta:
                if not frozenset(delta[(q, '')]) == frozenset():
                    ...
                    other_to_states = frozenset({ q for q in delta[(q, '')] if not q == to_state })
                    deltaprime = dict()
                    for q in Q:
                        if q == from_state:
                            if from_state == to_state:
                                ...
                                if not other_to_states == frozenset():
                                    if (to_state, '') in delta:
                                        T = other_to_states.union(delta[(to_state, '')])
                                        deltaprime[(q, '')] = frozenset({ q for q in T if not q == to_state })
                                    else:
                                        deltaprime[(q, '')] = other_to_states
                                else:
                                    if (to_state, '') in delta:
                                        ...
                                        deltaprime[(q, '')] = frozenset({ q for q in T if not q == to_state })
                                for a in Sigma:
                                    deltaprime[(q, a)] = delta[(q, a)].union(delta[(to_state, a)])
                        else:
                            if (q, '') in delta:
        ...
        def regular_expression_to_dfa_helper(expr, Sigma, l, r):
            ...
            if r < l:
                ...
                if expr[l] == '(':
                    ...
                    if m < l:
                        raise Exception("Could not find matching close parenthesis")
                    if m == r-1:
                        return regular_expression_to_dfa_helper(expr, Sigma, l+1, r-1)
                    else:
                        if expr[m+1] == '|':
                            left = regular_expression_to_dfa_helper(expr, Sigma, l, m+1)
                            right = regular_expression_to_dfa_helper(expr, Sigma, m+2, r)
                            ...
                            while (i < r) and (expr[i] == '*'):
                                i = i + 1
                            if i == r:
                                left = regular_expression_to_dfa_helper(expr, Sigma, l, r-1)
                                return left.star()
                            else:
                                if expr[i] == '|':
                                    left = regular_expression_to_dfa_helper(expr, Sigma, l, i)
                                    right = regular_expression_to_dfa_helper(expr, Sigma, i+1, r)
                                    return left.union(right)
                                else:
                                    left = regular_expression_to_dfa_helper(expr, Sigma, l, i)
                                    right = regular_expression_to_dfa_helper(expr, Sigma, i, r)
                                    return left.concat(right)
                        else:                            
                            left = regular_expression_to_dfa_helper(expr, Sigma, l, m+1)
                            right = regular_expression_to_dfa_helper(expr, Sigma, m+1, r)
                            return left.concat(right)
                else:
                    if expr[l] in { '|', '*' }:
                        raise Exception("Malformed expression")
                    if expr[l+1] == '|':
                        left = regular_expression_to_dfa_helper(expr, Sigma, l, l+1)
                        right = regular_expression_to_dfa_helper(expr, Sigma, l+2, r)
                        return left.union(right)
                    elif expr[l+1] == '*':
                        i = l+1
                        while (i < r) and (expr[i] == '*'):
                        ...
                        if i == r:
                            ...
                            while (j > l) and (expr[j] == '*'):
                                j = j - 1
                            left = regular_expression_to_dfa_helper(expr, Sigma, l, j)
                            right = regular_expression_to_dfa_helper(expr, Sigma, j, r)
                            return left.concat(right)
                        else:
                            if expr[i] == '|':
                                left = regular_expression_to_dfa_helper(expr, Sigma, l, i)
    ...
    def __convert_to_nfa(self):
        ...
        delta = { (frozenset({ q }), a) : frozenset({ frozenset({ self.__delta[(q,a)] }) }) for q in self.__states for a in self.__alphabet }
    ...
    def concat(self, other):
        ...
        for q in QB:
            for a in SigmaB:
                delta[((2,q),a)] = frozenset({ (2,r) for r in deltaB[(q,a)] })
        for q in FA:
